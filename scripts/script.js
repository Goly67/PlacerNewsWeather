...
async function updateNews() {
    ...
    const targetUrl = 'https://www.philstar.com/rss/';
    ...
    try {
        ...
        const parser = new DOMParser(); const API_KEY = '045ac4f01f984ff68820a4943e6592bd';
        const locationSelect = document.getElementById('locationSelect');
        const selectedLocation = document.getElementById('selectedLocation');
        const stormAlert = document.getElementById('stormAlert');
        const weatherIcon = document.getElementById('weatherIcon');
        ...
        async function updateNews() {
            ...
            try {
                ...
                const newsItems = Array.from(items).map(item => ({
                    ...
                    // Adjust the selector to match Philstar's RSS structure
                    image: item.querySelector('link[rel="image_src"]')?.getAttribute('href') || '', // Example
                ...
                newsGrid.innerHTML = limitedNewsItems.map(item => {
                    let imageHtml = '';
                    if (item.image) {
                        imageHtml = `<img src="${item.image}" alt="${item.title}" class="news-item-image">`;
                    }
                    return `
  <div class="news-item">
    ${imageHtml}
